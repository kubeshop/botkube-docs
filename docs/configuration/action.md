...
The action configuration allows you to automate your workflow by defining commands that are executed on an incoming event. Actions, similarly as communication platforms, use [source bindings](./source.md) to define the event sources (command triggers) and [executor bindings](docs/configuration/executor/kubectl.md) to run a given command.
