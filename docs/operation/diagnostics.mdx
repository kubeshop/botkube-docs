...
```bash
Used images
botkube:	ghcr.io/kubeshop/botkube:v1.3.0
cfg-watcher:	ghcr.io/kubeshop/k8s-sidecar:in-cluster-config
```

The `botkube` is the agent image. The container image tag (`v1.3.0`) is the version in which it was deployed on the cluster.
...
If the Engine is [healthy](#agent-health), you should be able to track any bug by checking the logs. To check the logs, run:
