...
4. Once added, browse to the channel and type `@Botkube ping` to make sure Botkube is responding.
   ...
   :::caution
   The MS Teams integration doesn't support persistence for notifications settings. As a result, when Botkube restarts, each time you need to send `@Botkube notifier start` to re-enable notifications.
   :::
